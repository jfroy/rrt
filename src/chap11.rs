...
pub fn chap11_scene<'a>(nx: usize, ny: usize) -> (Scene, Camera) {
    ...
    let look_from = Vec4f::new(3., 3., 2., 0.);
    let look_at = Vec4f::new(0., 0., -1., 0.);
    let focus_dist = (look_from - look_at).mag();
    let aperture = 2.;
    let camera = Camera::from(CameraCreateInfo {
        look_from,
        look_at,
        up: Vec4f::new(0., 1., 0., 0.),
        fov: 20.,
